...
function tagsFromName(name) {
    ...
    if (/business|conference|centraal station|central station/.test(n)) tags.push('business');
...
// Returns { excluded, exclusion_reason, exclusion_reasons, exclusion_confidence,
// source_signals } for a clear type-mismatch, or { excluded:false, downgrade } for a
// market-cluster mismatch (airport/resort) that is treated as excluded further down
// once combined with distance, or { excluded:false } when nothing applies.
...
// Weighted per spec: segment/chain-scale 30%, price band 25%, positioning 20%,
// location relevance 15%, scale (room count) fit 10%. Review score/volume are used
// only as a confidence signal elsewhere, never as a scoring driver here.
...
// Classifies and scores a single Google Places candidate against the subject
// hotel's profile. Returns a fully explainable candidate object - included
// candidates carry fit_score/fit_confidence/fit_reasons/weaknesses/data_gaps,
// excluded candidates carry exclusion_reason/exclusion_reasons instead.
export function classifyCandidate(place, hotel, subjectProfile, origin, radius) {
  ...
  // A hard market-cluster mismatch (airport/resort) that is also outside the
  // subject's relevance radius is excluded rather than left ambiguous - this is
  // what keeps a distant airport hotel out of a city-center luxury compset.
  ...
  // Obvious budget/economy candidate for a luxury/upper-upscale subject hotel -
  // only applied when the candidate's own scale is a confident brand match, to
  // avoid over-excluding on a noisy rating-only fallback.
  if (
